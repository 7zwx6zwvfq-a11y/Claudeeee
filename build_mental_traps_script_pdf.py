#!/usr/bin/env python3
"""PDF narration script for Video 11 — 3 Traps That Rewire Your Brain to Stay Broke."""
...
from build_mental_traps_script import SECTIONS
...
NUM  = ParagraphStyle('NUM',  parent=styles['Normal'], fontSize=9,  leading=12,
...
pdf_path = "/home/user/Claudeeee/Mental_Traps_SCRIPT.pdf"
...
flow = [
    ...
    Spacer(1, 4),
    Paragraph(f"SCRIPT (NARRATION) · {total_beats} beats · ~{word_count} words · ~{round(word_count/140)} min", META),
    Spacer(1, 14),
]
...
for section_title, lines in SECTIONS:
    ...
    for line in lines:
        flow.append(Paragraph(f'<font color="#888888"><b>[{beat_num}]</b></font>  {esc(line)}', LINE))
        beat_num += 1

    flow.append(Spacer(1, 8))

flow.append(Spacer(1, 10))
flow.append(Paragraph(f"END OF SCRIPT · {total_beats} beats · ~{word_count} words · ~{round(word_count/140)} min", META))
